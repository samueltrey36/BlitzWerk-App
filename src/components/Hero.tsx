        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link to="/create-account" onClick={() => { sessionStorage.setItem('intendedRole', 'Customer'); sessionStorage.setItem('flowReturnTo', '/get-help'); }} className="bg-brand text-white text-lg font-bold px-8 py-5 rounded-2xl shadow-xl shadow-brand/30 hover:scale-105 transition-all active:scale-95 flex items-center justify-center gap-3 group">
                Get Help Now
                <Navigation className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/create-account" onClick={() => { sessionStorage.setItem('intendedRole', 'Helper'); sessionStorage.setItem('flowReturnTo', '/become-helper'); }} className="bg-white text-slate-900 text-lg font-bold px-8 py-5 rounded-2xl border-2 border-slate-200 hover:border-brand hover:text-brand transition-all active:scale-95 flex items-center justify-center gap-3">
                Become a Helper
              </Link>
            </div>
          </motion.div>
